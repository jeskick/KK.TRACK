...
void BNO08x_init(BNO08x *device, BNO08x_config_t *imu_config)
{
    ...
    esp_err_t add_ret = spi_bus_add_device(imu_config->spi_peripheral, &(device->imu_spi_config), &(device->spi_hdl));
    if (add_ret != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to add SPI device: %s", esp_err_to_name(add_ret));
        return;
    }
